...
The `home-routes` handler in the `luminus-personas.routes.home` namespace
...
The `home-page` function will in turn call the `luminus-personas.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `luminus-personas.handler` namespace:
...
Request middleware functions are located under the `luminus-personas.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `luminus-personas.dev-middleware` namespace found in
